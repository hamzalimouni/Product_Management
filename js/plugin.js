...
// create product and save localestorage and count
let dataPro;
...
submit.onclick = () => {
    ...
    if (title.value != '' && price.value != '' && category.value != '' && newPro.count < 10) {
        ...
        alert('Verifié les informations avant de crée ou modifier un produit !!');
    }
...
function showData() {
    ...
    if (dataPro.length > 0) {
        ...
        <button onclick="deleteAllItems()">Delete All (${dataPro.length})</button>
        `;
    }
    else {
        deleteAllData.innerHTML = '';
...
function deleteItem(i) {
    if (window.confirm("vous voulez supprimer ce produit ?") == true) {
        dataPro.splice(i, 1);
...
function deleteAllItems() {
    if (window.confirm('vous voulez supprimer tout les produit ?') == true) {
        // firts method
        // dataPro = [];
        // localStorage.product = JSON.stringify(dataPro); 
        // second method
